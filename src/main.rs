...
use std::{rc::Rc, time::{self, Duration}};
use rand::Rng;
use crate::{heap::{heapsort_fast_construct, heapsort_slow_construct}, merge::mergesort};
fn main() {
    let n = 1000000;
    let loop_n = 10;
    let vec = gen_random_vec(n);

    track_avr(heapsort_slow_construct, &vec, "heapsort_slow_construct", loop_n);
    // track_avr(mergesort, &vec, "mergesort", loop_n);
}

fn track_avr(to_track : impl Fn(&[i32])->Vec<i32>, data : &[i32], label : &str, loop_n : usize) -> Duration {
